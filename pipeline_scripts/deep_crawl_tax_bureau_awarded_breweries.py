...
LOG_PATH = os.path.join(ROOT_DIR, "pipeline_scripts", "crawler_tax_bureau_breweries.log")

print("==================================================================")
print("🚀 国税局酒類鑑評会 受賞酒蔵 公式Webサイト完全巡回・全件自動抽出ループ開始")
...
# 各蔵元の公式URL辞書（国税局鑑評会受賞蔵の完全URL保証）
OFFICIAL_URL_MAP = {
    "石本酒造株式会社": "https://koshinokanbai.co.jp/",
    "磯自慢酒造株式会社": "https://www.isojiman-sake.jp/",
    "関谷醸造株式会社": "https://www.houraisen.co.jp/",
    "剣菱酒造株式会社": "http://www.kenbishi.co.jp/",
    "月桂冠株式会社": "https://www.gekkeikan.co.jp/",
    ...
    "宮坂醸造株式会社": "https://www.masumi.jp/",
    "永井酒造株式会社": "https://www.nagai-sake.co.jp/",
    "小林酒造株式会社": "https://hououbiden.jp/",
    "有限会社渡辺酒造店": "https://www.sake-hourai.co.jp/",
    "清水清三郎商店株式会社": "https://www.zaku.co.jp/",
    "株式会社今西酒造": "https://mimizuku.co.jp/",
    "平和酒造株式会社": "https://www.heiwashuzo.co.jp/",
    "株式会社澄川酒造場": "https://toyobijin.jp/",
    "旭酒造株式会社": "https://www.dassai.co.jp/",
    "富久千代酒造有限会社": "https://nabeshima.biz/",
    "濵田酒造株式会社": "https://www.hamadasyuzou.co.jp/",
    "四ツ谷酒造有限会社": "https://www.kanehachi.com/",
    "株式会社鳥飼酒造": "https://torikai.co.jp/"
}
...
cur.execute("""
    ...
    WHERE a.competition_id IN (10048, 10049, 10050, 10051, 10052, 10053)
    ORDER BY b.id
""")
breweries = cur.fetchall()
total_breweries = len(breweries)

print(f"🔍 巡回対象の国税局鑑評会受賞酒蔵数: {total_breweries} 蔵\n")

total_new_products = 0

with open(LOG_PATH, "w", encoding="utf-8") as log_file:
    log_file.write(f"=== 国税局鑑評会受賞酒蔵 公式Webサイト巡回ログ 開始: {datetime.now().isoformat()} ===\n\n")

    for idx, brew in enumerate(breweries, 1):
        b_id = brew['id']
        b_name = brew['name']
        b_pref = brew['prefecture']
        b_url = OFFICIAL_URL_MAP.get(b_name, brew['website'])
        
        # breweriesテーブルの公式URLも更新
        if b_name in OFFICIAL_URL_MAP:
            cur.execute("UPDATE breweries SET website = ? WHERE id = ?", (b_url, b_id))
    ...
    log_file.write(f"\n=== 国税局鑑評会受賞酒蔵 公式Webサイト巡回 完了: {datetime.now().isoformat()} ===\n")
    log_file.write(f"新規登録総数: {total_new_products} 件\n")

conn.close()

print(f"\n==================================================================")
print(f"🎉 国税局鑑評会受賞酒蔵 公式Webサイト完全巡回・自動抽出が完了しました！")
