...
LOG_PATH = os.path.join(ROOT_DIR, "pipeline_scripts", "crawler_110_breweries.log")

print("==================================================================")
print("🚀 110酒蔵 公式Webサイト完全巡回・全件自動抽出ループ パイプライン開始")
...
total_updated_specs = 0

with open(LOG_PATH, "w", encoding="utf-8") as log_file:
    log_file.write(f"=== 110酒蔵 公式Webサイト巡回ログ 開始: {datetime.now().isoformat()} ===\n\n")
    ...
    for idx, brew in enumerate(breweries, 1):
        ...
        if not b_url or not b_url.startswith("http"):
            skip_msg = f"  ⚠️ [スキップ] 有効なURLがありません: {b_url}"
            print(skip_msg)
        ...
        visited_pages = set()
        extracted_products_in_brewery = 0

        # 代表ページおよび商品一覧ページを順次チェック
        for path in PRODUCT_PATH_CANDIDATES[:5]: # 主要パスを探索
            ...
            for line in lines:
                ...
                # 日本酒・本格焼酎・リキュール・クラフトサケの特徴語
                is_product_match = False
                matched_category = "普通酒"
                
                if any(k in line for k in ["純米大吟醸", "純米吟醸", "特別純米", "大吟醸", "吟醸", "特別本醸造", "本醸造", "純米酒", "生酛", "山廃", "スパークリング", "梅酒", "ゆず酒", "リキュール", "クラフトサケ", "本格焼酎", "泡盛", "芋焼酎", "麦焼酎", "米焼酎", "黒糖焼酎", "どぶろく", "ひやおろし", "しぼりたて", "生原酒", "にごり酒"]):
                ...
                if is_product_match:
                    ...
                    if not existing:
                        ...
                        try:
                            ...
                            extracted_products_in_brewery += 1
        ...
        conn.commit()
        summary_line = f"  👉 {b_name}: {extracted_products_in_brewery} 件の新規製品を公式Webから自動抽出・登録完了\n"
        print(summary_line)
        log_file.write(summary_line)

    log_file.write(f"\n=== 全110酒蔵 公式Webサイト巡回 完了: {datetime.now().isoformat()} ===\n")
    log_file.write(f"新規登録総数: {total_new_products} 件\n")

conn.close()

print(f"\n==================================================================")
print(f"🎉 110酒蔵 公式Webサイト完全巡回・全件自動抽出ループが完了しました！")
